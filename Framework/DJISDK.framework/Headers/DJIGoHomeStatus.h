...
/**
 *  The estimated remaining time, in seconds, it will take the aircraft to go
 *  home with a 10% battery buffer remaining.
...
/**
 *  The estimated time, in seconds, needed for the aircraft to land from its
 *  current height. The time calculated will be for the aircraft to land, moving
 ...
 *  aircraft to go home and have 10% battery remaining. This includes landing of
 *  the aircraft.
 */
 ...
 *  the aircraft will automatically go back to its home location. This can be
 *  canceled at any time with the `cancelGoHome` method (which will also clear
 *  `aircraftShouldGoHome`). It is recommended that an alert view is shown to
 *  the user when `aircraftShouldGoHome` returns YES.
 ...
 *
 *  The two main situations in which `aircraftShouldGoHome` will return YES are
 *  if the aircraft's battery is too low or if the aircraft has flown too far away.
 */
