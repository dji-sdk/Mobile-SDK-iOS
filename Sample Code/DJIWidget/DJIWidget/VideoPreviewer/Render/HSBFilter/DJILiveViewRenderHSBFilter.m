...
@implementation DJILiveViewRenderHSBFilter {
    float matrix[4][4];
}
...
- (void) setRotateHue:(float)rotateHue{
    
    if (_rotateHue == rotateHue) {
        return;
    }
    
    _rotateHue = rotateHue;
    [self _updateColorMatrix];
}
...
- (void)_updateColorMatrix {
    ...
    huerotatemat(matrix, _rotateHue);
    saturatemat(matrix, _saturation);
    cscalemat(matrix, _brightness, _brightness, _brightness);
    
    DJIGPUMatrix4x4 gpuMatrix;
